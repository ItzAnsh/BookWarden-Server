...
const createMultipleLibrarians = AsyncErrorHandler(async (req, res) => {
  const librarians = req.body.librarians;
  ...
  for (const librarianData of librarians) {
    const { name, email } = librarianData;
    ...
    createdLibrarians.push({ name, email, password });
  }
...
export { modifyUser, getAllUsers, createLibrary, createLibrarian, createMultipleLibrarians };
