...
const sendIssueStatusEmail = async (to, issue) => {
  // Determine if there are multiple books or a single book
  const bookTitles = issue.books.map(book => book.title).join(", ");
  const bookMessage = issue.books.length > 1 ? "books" : "book";

  // Generate the email content
  const htmlContent = `
    <p>Hello ${issue.userId.name},</p>
    <p>Your ${bookMessage} <strong>${bookTitles}</strong> ${issue.books.length > 1 ? 'have' : 'has'} been ${issue.status}.</p>
  `;
...
export {sendWelcomeEmail, sendIssueStatusEmail};
